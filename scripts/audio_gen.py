...
def inject_break_tags(text: str) -> str:
    """
    Returns clean natural text without injecting rigid digital SSML break tags.
    Modern ElevenLabs Multilingual v2 models rely on organic punctuation (commas,
    ellipses '...', and dashes) for human prosody and breath control. Artificial
    0.3s/0.15s tags at every comma create a robotic GPS-navigation effect.
    """
    # Clean up double spaces while leaving organic punctuation intact
...
def apply_mastering_chain(audio_path: str, is_elevenlabs: bool = True) -> None:
    ...
    if is_elevenlabs:
        ...
        # Warm, natural human broadcast profile (warm chest body, smooth presence, no harsh sibilance)
        filter_str = (
            "highpass=f=75,"  # removes sub-audible room rumble below 75Hz
            "equalizer=f=250:t=h:w=120:g=+1.0,"  # warm chest resonance
            "equalizer=f=3500:t=h:w=1500:g=+1.0,"  # gentle vocal presence without sibilance
            "acompressor=threshold=-18dB:ratio=2.5:attack=10:release=80,"  # transparent musical compression
...
def _generate_f5_clone(text, output_path):
    ...
    # Combine segments with 60ms cross-fade for smooth, natural speech flow
    CROSSFADE_MS = 60
    from pydub import AudioSegment
