...
def create_video(audio_path, script_json, chunks, output_path=None):
    ...
    if ENABLE_WATERMARK:
        ...
        header_font = get_font_for_text("Tamil Tips by VJ", 38, "bold")
        text_x = 50
        # Draw premium semi-translucent text watermark with dark drop shadow (readable on any background)
        header_draw.text((text_x + 2, 82), "Tamil Tips by VJ", fill=(10, 10, 15, 180), font=header_font)
        header_draw.text((text_x, 80), "Tamil Tips by VJ", fill=(255, 255, 255, 140), font=header_font)
